namespace :db do
    ...
    task up: :environment do
        user = User.create(name: 'avatar', email: 'test@test.ru', moderator: true, creator: true)
        post = Post.create(title: 'test title', body: TEST_BODY, user: user, visible: true)
        comment = Comment.create(user: user, post:post, body: 'asdadadasdasfdasda', visible: true )
    end

    task down: :environment do
        Comment.where(body: 'asdadadasdasfdasda').delete_all
        Post.where(title: 'test title').delete_all
        User.where(email: 'test@test.ru').delete_all
