...
initCreateAppForm: function(hash) {
  ...
  cur.dev_create_app_select = new Dropdown(id_el, cur.dev_create_app_apps !== false ? cur.dev_create_app_apps : [[-1, getLang('pages_widget_auth_required')]], {
    width: 200,
    big: 1,
  ...
  cur.dev_create_app_emitter.addListener('change', function(app_id) {
    if(app_id && app_id > 0) {
      ...
      nav.setLoc(extend(nav.objLoc, {aid: app_id}));
    }
  });
...
initWidgetAppConstructor: function(options) {
  ...
  Emoji.init(width_el, {noLineBreaks: true, noCtrlSend: 1, onSend: updateWidth});
...
initWidgetAuthConstructor: function(options) {
  ...
  VK.init({apiId: options.app_id_default});
  ...
  function updateCode() {
    ...
    if (state.mode == 2) {
      ...
      _options.onAuth = function(data) {alert('user '+data['uid']+' authorized');};
      tpl = rs(tpl, rep);
    }
    val(code_el, tpl);
...
initWidgetArticleConstructor: function(options) {
  var wArticle = {
    ...
    updateCode: function(article, url) {
      ...
      VK.Widgets.Article("vk_article_" + data.owner_id + "_" + data.article_id, data.article_url, { base_domain: options.base_domain });
    }
  };
